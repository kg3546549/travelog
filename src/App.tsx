...
import { Plane, Check, Waves, ShoppingBag, DollarSign, ChevronLeft, MapPin, Calendar, ExternalLink, RefreshCw } from 'lucide-react'
      ...
      <main className="w-full max-w-[720px] px-4 pt-8 space-y-9">
        <section>
          ...
          <Card className="rounded-xl border-black/5 overflow-hidden shadow-sm bg-white">
            <div className="divide-y divide-black/5">
              {[
                { type: 'out', from: '인천', to: '사이판', time: '07/07 (화) 20:20 → 07/08 02:00 도착', carrier: '제주항공', icon: '🛫' },
                { type: 'in', from: '사이판', to: '인천', time: '07/12 (일) 03:05 → 07/12 06:50 도착', carrier: '제주항공', icon: '🛬' }
              ].map((flight, i) => (
                <div key={i} className="flex items-center gap-4 p-5">
                  <div className={cn("w-9 h-9 rounded-full flex items-center justify-center text-xl", flight.type === 'out' ? "bg-teal-50" : "bg-amber-50")}>{flight.icon}</div>
                  <div className="flex-grow min-w-0">
                    <div className="text-sm font-medium">{flight.from} → {flight.to}</div>
                    <div className="font-mono text-xs text-gray-600 mt-1">{flight.time}</div>
                  </div>
                  <Badge className="bg-teal-50 text-teal-800 text-[10px] border-none">{flight.carrier}</Badge>
                </div>
              ))}
            </div>
          </Card>
